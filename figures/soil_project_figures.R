...
# Version: interactive file selection
# ============================================================
...
# ============================================================
# Load data via file dialogs
# ============================================================
cat("=== SOC Paper Figures ===\n")
cat("Please select source data files when prompted.\n\n")

cat("[1/7] Select fig1_study_area_profiles.csv\n")
fig1 <- read.csv(choose_csv("采样点数据（剖面坐标+气候区）"))

cat("[2/7] Select fig2a_depth_distribution.csv\n")
fig2a <- read.csv(choose_csv("深度分布数据"))

cat("[3/7] Select fig2b_soc_by_depth.csv\n")
fig2b <- read.csv(choose_csv("SOC按深度分组数据"))

cat("[4/7] Select fig2c_landcover.csv\n")
fig2c <- read.csv(choose_csv("土地覆盖类型数据"))

cat("[5/7] Select fig3_formal_aggregated.csv\n")
fig3 <- read.csv(choose_csv("模型性能汇总数据"))

cat("[6/7] Select fig4_grid_stats.csv\n")
fig4 <- read.csv(choose_csv("网格统计数据"))

cat("[7/7] Select fig5_delta_r2.csv\n")
fig5 <- read.csv(choose_csv("ΔR²数据"))

cat("\nAll data loaded. Generating figures...\n\n")
...
# ============================================================
# Fig 3: Model Performance Comparison (2 panels)
# ============================================================
...
depth_df <- fig3 %>%
  ...
  select(model, starts_with("R2_") & !starts_with("R2_all") & !starts_with("R2_surface") & !starts_with("R2_deep")) %>%
...
# Fig 5: ΔR²
# ============================================================
depth_group_labels <- c("all" = "All depths", "surface" = "Surface (0-30 cm)", "deep" = "Deep (>30 cm)")
...
cat("Files:\n")
cat("  fig2_data_overview.png/pdf\n")
cat("  fig3_performance.png/pdf\n")
cat("  fig4_spatial_grid.png/pdf\n")
cat("  fig5_delta_r2.png/pdf\n")
